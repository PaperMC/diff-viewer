...
{#snippet blacklistPopoverContent()}
  ...
  <div class="flex items-center gap-1 px-2">
    ...
    >
      <input
        bind:value={instance.dirBlacklistInput}
        type="text"
        class="w-full rounded-l-md border-t border-b border-l px-2 py-1 inset-shadow-xs"
      />
...
{#snippet githubSection()}
  <section class="flex flex-col p-4">
    ...
    <div class="mb-2 flex flex-row gap-1">
      ...
      {:else}
        <Button.Root
          class="flex w-fit flex-row items-center justify-between gap-2 rounded-md btn-fill-neutral px-2 py-1"
        ...
        <InfoPopup>
          Sign in to GitHub for higher rate limits and private repository access. Only private repositories configured
          for the GitHub app will be accessible.
        </InfoPopup>
      {/if}
    </div>
    ...
    <div class="flex flex-row gap-1">
      ...
      <InfoPopup>
        In order to view a private repository, the repository owner must have installed the GitHub app and granted it
        access to the repository. Then, authenticated users will be able to load diffs they have read access to.
      </InfoPopup>
    </div>
  </section>
{/snippet}
...
{#snippet patchSection()}
  ...
  >
    ...
    <MultimodalFileInput
      bind:state={instance.patchFile}
      required
      fileTypeOverride={false}
      defaultMode="file"
      label="Patch File"
    />
...
{#snippet filesSection()}
  ...
  >
    <h3 class="mb-4 flex items-center gap-1 text-lg font-semibold">
      ...
      From Files
      <InfoPopup>
        Compares any two texts or images. For files, the file type (text or image, and language) is inferred from the
        file extension and content. The file type can be overridden for text to control syntax highlighting.
      </InfoPopup>
    </h3>
...
{#snippet directoriesSection()}
  ...
  >
    <h3 class="mb-4 flex items-center gap-1 text-lg font-semibold">
      ...
      From Directories
      <InfoPopup>
        Compares the entire contents of the directories, including subdirectories. Does not attempt to detect renames.
        When possible, preparing a unified diff (<code class="rounded-sm bg-neutral-2 px-1 py-0.5">.patch</code> file) using
        Git or another tool and loading it with the above button should be preferred.
      </InfoPopup>
    </h3>
    <div class="mb-2 flex w-full flex-col gap-1">
      {#each instance.flipDirs as id, index (id)}
        <div animate:flip={{ duration: 250 }} class="flex">
          {#if id === "1"}
            <DirectorySelect
              bind:directory={instance.dirOne}
              placeholder={index === 0 ? "Directory A" : "Directory B"}
            />
          {:else if id === "arrow"}
            <span class="iconify size-4 shrink-0 octicon--arrow-down-16"></span>
          {:else if id === "2"}
            <DirectorySelect
              bind:directory={instance.dirTwo}
              placeholder={index === 2 ? "Directory B" : "Directory A"}
            />
          {/if}
        </div>
      {/each}
