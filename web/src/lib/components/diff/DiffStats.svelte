...
{#snippet ratio()}
  ...
  <span class="text-green-600 dark:text-green-400">+{ratioAdd}</span>:<span class="text-red-600 dark:text-red-400"
    >-{ratioRemove}</span
  >
{/snippet}
    ...
    <Tooltip.Trigger>
      <div class="relative h-3 w-12 bg-neutral-2">
        ...
        {:else}
          ...
          ></div>
          <div
            class="absolute top-0 h-3 border border-red-700 bg-red-600 dark:border-red-600 dark:bg-red-400"
            style={getRemoveBarStyle()}
          ></div>
        {/if}
      </div>
    </Tooltip.Trigger>
