...
{#snippet modeButton(forMode: Mode, iconClass: string)}
  <RadioGroup.Item
    value={forMode}
    class="flex items-center justify-center rounded-sm btn-ghost px-2 py-1 data-[state=checked]:btn-ghost-visible"
  >
...
{#snippet modeSelector()}
  <RadioGroup.Root
    ...
    bind:value={mode}
    class="mb-4 flex flex-row gap-1 rounded-lg bg-neutral p-1.5 shadow-sm"
  >
    ...
    <div class="relative grid grid-cols-1 gap-4" style={overlayContainerStyle}>
      ...
      <div class="absolute flex size-full">
        ...
        >
          ...
          />
          <span class="absolute h-full w-0.5 -translate-x-1/2 bg-em-disabled/80" style="left: calc({slidePercent}%);"
          ></span>
          <Slider.Thumb index={0} class="group absolute flex h-full cursor-col-resize select-none">
            <div
              class="flex place-self-center rounded-sm bg-neutral px-0.5 py-1 shadow-sm group-data-active:scale-[0.95]"
            >
...
{#snippet fade(dims: DimensionData)}
  ...
  <div class="sticky bottom-4 mt-4 flex w-full max-w-[280px] items-center rounded-lg bg-neutral p-2.5 shadow-sm">
    ...
    >
      ...
      <Slider.Thumb
        index={0}
        class="block size-4 cursor-pointer rounded-full border bg-neutral shadow-sm transition-colors data-active:scale-[0.95]"
      />
    </Slider.Root>
  </div>
{/snippet}
